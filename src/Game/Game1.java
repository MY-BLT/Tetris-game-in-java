...
public class Game1 {
    ...
    public void play() {
        ...
        Position leftShape = new Position(board.getStartColumn());
        Position topShape = new Position(board.getStartRow());
        ...
        do {
            ...
            try {
                switch (action) {
                    case MOVE_LEFT:
                        boolean canMoveLeft = checkMoveMent(MOVE_LEFT, currentBoard, topShape.getValue(),
                                leftShape.getValue(), row, column);
                        if (canMoveLeft) {
                            isMoveLeft = moveLeft(currentBoard);
                        }
                        break;
                    case MOVE_RIGHT:
                        boolean canMoveRight = checkMoveMent(MOVE_RIGHT, currentBoard, topShape.getValue(),
                                leftShape.getValue(), row, column);
                        if (canMoveRight) {
                        ...
                        break;
                    case ROTATE:
                        boolean canRotate = checkMoveMent(ROTATE, currentBoard, topShape.getValue(),
                                leftShape.getValue(), row, column);
                        if (canRotate)
            ...
            if (isMoveLeft)
                leftShape.reduceValue();
            if (isMoveRight)
                leftShape.increaseValue();
            if (isRotate) {
                row = row + column;
                column = row - column;
                row = row - column;
            }
            boolean canMoveDown = checkMoveMent(MOVE_DOWN, currentBoard, topShape.getValue(), leftShape.getValue(), row, column);
            if (canMoveDown) {
                isMoveDown = moveDown(currentBoard, canMoveDown);
                canMove = isMoveDown;
            }
            if (isMoveDown)
                topShape.increaseValue();

            if (topShape.getValue() + row == height - 1) {
                canMove = false;
            } else {
                for (int i = topShape.getValue(); i < topShape.getValue() + row; i++) {
                    for (int j = leftShape.getValue(); j < leftShape.getValue() + column; j++) {
            ...
            if (!canMove) {
                firmShape(currentBoard, topShape.getValue());
                ...
                if (!isGameOver) {
                    ...
                    leftShape.setValue(board.getStartColumn());
                    topShape.setValue(board.getStartRow());
                }
            }
        } while (!isGameOver);
    }
    ...
    /**
     * The function `checkMovement` in Java checks if a specific move action is
     * valid on a game board
     * based on the current state and position of shapes.
     * 
     * @param action       The `action` parameter in the `checkMovement` method
    ...
    */
     ...
     *         returns `false`.
     */
    private boolean rotate(int[][] currentBoard, Position left, Position top, int row, int column) {

        int bottom = top.getValue() + row - 1;
        int right = left.getValue() + column - 1;
        int xCenter = top.getValue();
        int yCenter = left.getValue();
        boolean isRotate = false;
        int[][] shape = new int[column][row];

        for (int i = top.getValue(); i < bottom + 1; i++) {
            for (int j = left.getValue(); j < right + 1; j++) {
                int xNew = xCenter - yCenter + j;
                int yNew = xCenter + yCenter - i + row - 1;
                int x = xNew - top.getValue();
                int y = yNew - left.getValue();
        ...
        for (int i = top.getValue(); i < top.getValue() + column; i++) {
            for (int j = left.getValue(); j < left.getValue() + row; j++) {
                currentBoard[i][j] = shape[i - top.getValue()][j - left.getValue()];
                isRotate = true;
            }
        }
        // if (isRotate) {
